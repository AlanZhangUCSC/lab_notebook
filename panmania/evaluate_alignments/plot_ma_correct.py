...
with open(diff_file, 'r') as f:
  next(f)
  for line in f:
    parent, child, panman_diff, mafft_diff, expected_diff_after_correction, mismatch_count_in_ma_blocks, mismatch_count_in_corrected_blocks, all_mismatch_count = line.strip().split('\t')
    assert(int(panman_diff) == int(all_mismatch_count))
    pairs_dict[parent][child] = (int(panman_diff), int(mafft_diff), int(expected_diff_after_correction), int(mismatch_count_in_ma_blocks), int(mismatch_count_in_corrected_blocks), int(all_mismatch_count))
...
for parent in pairs_dict.keys():
  for child in pairs_dict[parent].keys():
    panman_diff, mafft_diff, expected_diff_after_correction, mismatch_count_in_ma_blocks, mismatch_count_in_corrected_blocks, all_mismatch_count = pairs_dict[parent][child]
    mismatches_all = panman_diff - mafft_diff
    mismatches_after_correction = expected_diff_after_correction - mafft_diff
...
diff_distribution_panel.set_xlim(0, 11-0.2)
diff_distribution_panel.set_ylim(0, (max(max(diff_bins), max(diff_bins_after_correction)) // 100 + 1) * 100)
for i in range(11):
