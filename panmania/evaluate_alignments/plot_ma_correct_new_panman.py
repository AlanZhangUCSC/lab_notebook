...
with open(diff_file, 'r') as f:
  next(f)
  for line in f:
    parent, child, panman_diff, mafft_diff, expected_diff_after_correction, mismatch_count_in_ma_blocks= line.strip().split('\t')
    try:
      pairs_dict[parent][child] = (int(panman_diff), int(mafft_diff), int(expected_diff_after_correction), int(mismatch_count_in_ma_blocks))
    except:
      continue
...
for parent in pairs_dict.keys():
  for child in pairs_dict[parent].keys():
    panman_diff, mafft_diff, expected_diff_after_correction, mismatch_count_in_ma_blocks = pairs_dict[parent][child]
    mismatches_all = panman_diff - mafft_diff
    mismatches_after_correction = expected_diff_after_correction + mismatch_count_in_ma_blocks - mafft_diff
