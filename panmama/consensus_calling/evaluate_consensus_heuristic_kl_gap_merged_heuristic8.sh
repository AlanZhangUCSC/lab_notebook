...
#SBATCH --job-name=sars_evaluate_consensus_heuristic_8
...
command_text=${consensus_output_dir}/heuristic8_command_text.txt
...
for ((i=0; i<${#estimated_haplotypes[@]}; i++)); do
  ...
  (
    ...
    samtools calmd "${consensus_output_prefix}.sorted.bam" "$ref_fasta" > "${consensus_output_prefix}.sorted.calmd.sam"
    samtools view -b "${consensus_output_prefix}.sorted.calmd.sam" > "${consensus_output_prefix}.sorted.calmd.bam"
    samtools index "${consensus_output_prefix}.sorted.calmd.bam"
    
    bcftools mpileup -f "$ref_fasta" "${consensus_output_prefix}.sorted.calmd.bam" \
    ...
    python3 /private/groups/corbettlab/alan/panmama-snakemake/workflow/evals/scripts/subconsensus/gen_pcf.py \
      ...
      --bam "${consensus_output_prefix}.sorted.calmd.bam" \
      --abundances "${merged_abundance_file}" \
      --assignments "${assigned_reference_file}" \
      --fasta "$aligned_fasta" \
      --min_coverage 10 \
      --min_allele_depth 5,0.02 \
      --error_rate 0.05 \
...
for ((i=0; i<${#estimated_haplotypes[@]}; i++)); do
    ...
    all_sorted_bams="${all_sorted_bams} ${consensus_output_prefix}.sorted.calmd.bam"
    all_pcfs="${all_pcfs} ${consensus_output_prefix}.pcf"
    all_output="${all_output} ${consensus_output_prefix}.heuristic_kl_gap_merged_heuristic8.varonly.vcf"

done

cmd="python3 /private/groups/corbettlab/alan/panmama-snakemake/workflow/evals/scripts/subconsensus/heuristic_v8.py \
  --vcfs ${all_vcf_gzs} \
...
for ((i=0; i<${#estimated_haplotypes[@]}; i++)); do
  ...
  rm "${consensus_output_prefix}.heuristic_kl_gap_merged_heuristic8.varonly.vcf.gz"
  rm "${consensus_output_prefix}.heuristic_kl_gap_merged_heuristic8.varonly.vcf.gz.csi"
  bgzip "${consensus_output_prefix}.heuristic_kl_gap_merged_heuristic8.varonly.vcf"

  bcftools index "${consensus_output_prefix}.heuristic_kl_gap_merged_heuristic8.varonly.vcf.gz"
  ...
  bcftools consensus -f "$ref_fasta" \
      -H 1 \
      "${consensus_output_prefix}.heuristic_kl_gap_merged_heuristic8.varonly.vcf.gz" \
...
comparison_output="${consensus_output_dir}/consensus_to_true_comparison_heuristic_kl_gap_merged_heuristic8.tsv"
