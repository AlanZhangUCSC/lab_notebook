...
cmd=(docker run --rm
  ...
  -v "$(realpath $(dirname $PMI)):/pmais"
  -v "$(realpath $(dirname $OUT_PREFIX)):/output"
  -w /panmap
  --user "$(id -u):$(id -g)"
  panmap-dev
  bash -c "/panmap/build/bin/panmap /panmans/$(basename $PANMAN) \
           -m /pmais/$(basename $PMI) \
           --dump-random-node-clusters-leaves $cluster_sizes_string \
...
selected_cluster_file=${OUT_PREFIX}.randomClusterIDs.tsv
random_nodes_file=${OUT_PREFIX}.randomNodeIDs.txt
touch $random_nodes_file

cat $selected_cluster_file  | tail -n+2 | cut -f 1 > $random_nodes_file
rm $selected_cluster_file

abundance_file=${OUT_PREFIX}.abundance.txt
touch $abundance_file

abundances=($(printf "0.01 %.0s" {1..100}))
paste "$random_nodes_file" <(printf '%s\n' "${abundances[@]}") > "$abundance_file"
