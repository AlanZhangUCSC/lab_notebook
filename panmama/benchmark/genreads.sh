...
num_mutated=$(awk "BEGIN {print int($NUMHAP * $PERMUT)}")
num_unmutated=$(($NUMHAP - num_mutated))
...
if [[ "$SEQTYPE" == "shotgun" ]]; then
  ...
  cmd="ivar trim -e -q 1 -i ${sorted_bam} -b ${REFERENCE_PRIMER_BED_FILE} -p ${trimmed_bam}"
  echo "$cmd" >> "$logfile"
  if [[ $TEST_COMMANDS -eq 0 ]]; then
    $cmd
  fi
