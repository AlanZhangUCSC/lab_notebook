...
parser.add_argument('input_prefix', help='Directory containing *.mgsr.assignedReadsLCANode.annotated.out and matching *.mgsr.assignedReads.fastq files')
...
fastq_path = input_prefix + '.mgsr.assignedReads.fastq'
ann_path = input_prefix + '.mgsr.assignedReadsLCANode.out'

if not os.path.isfile(fastq_path):
  raise FileNotFoundError(f'Error: {fastq_path} not found')
  exit(1)
if not os.path.isfile(ann_path):
  raise FileNotFoundError(f'Error: {ann_path} not found')
  exit(1)
