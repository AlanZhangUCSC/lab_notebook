...
parser.add_argument('-c', '--color-by', type=str, default='lca_count', choices=['read_count', 'fractionated_read_count', 'lca_count', 'lca_subtree_count', 'None'], help='Color by read count or LCA count')
parser.add_argument('-s', '--size-by', type=str, default='lca_subtree_count', choices=['read_count', 'fractionated_read_count', 'lca_count', 'lca_subtree_count', 'None'], help='Size by read count or LCA count')
parser.add_argument('-a', '--above-branch-label', type=str, default='lca_count', choices=['read_count', 'fractionated_read_count', 'lca_count', 'lca_subtree_count', None], help='Above branch label')
parser.add_argument('-b', '--below-branch-label', type=str, default='lca_subtree_count', choices=['read_count', 'fractionated_read_count', 'lca_count', 'lca_subtree_count', None], help='Below branch label')
...
reads_assigned = defaultdict(list)

with open(args.node_read_counts_file, 'r') as f:
  for line in f:
    node_label, _, count, assigned_reads = line.strip().split('\t')
    for node_label_clean in node_label.split(','):
      ...
      reads_assigned[node_label_clean] = list(map(int, assigned_reads.split(',')))

read_assigment_counts = defaultdict(int)
for node_label, reads in reads_assigned.items():
  for read in reads:
    read_assigment_counts[read] += 1

node_read_counts_fractionated = defaultdict(int)

for node_label, reads in reads_assigned.items():
  for read in reads:
    node_read_counts_fractionated[node_label] += 1 / read_assigment_counts[read]

max_node_read_counts_fractionated = max(node_read_counts_fractionated.values())
min_node_read_counts_fractionated = min(node_read_counts_fractionated.values())
...
for node in tree.postorder_node_iter():
  ...
  elif args.color_by == 'fractionated_read_count':
    branch_color = get_node_color(node_read_counts_fractionated.get(node_label, 0), max_node_read_counts_fractionated, min_node_read_counts_fractionated, args.gamma)
  else:
    branch_color = '#D3D3D3'
...
text_pad_data_y = 1.0 / pts_per_data_y
...
for node_label in node_coord:
  ...
  fractionated_read_count = node_read_counts_fractionated.get(node_label, 0)
  ...
  if args.color_by == 'read_count':
    ...
    color = get_node_color(fractionated_read_count, max_node_read_counts_fractionated, min_node_read_counts_fractionated)
  else:
    color = '#D3D3D3'
  ...
  panel.text(
    ...
    ny + text_pad_data_y,
    f'{read_count} | {fractionated_read_count:.1f} | {lca_count} | {lca_subtree_count}',
    fontfamily='sans',
